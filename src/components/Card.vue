<template>
  <div class="flex gap-6">
    <div
      v-for="stat in stats"
      :key="stat"
      class="flex flex-col p-4 mr-2 mt-5 w-1/3 bg-red-900 rounded-lg gap-y-3"
    >
      <div class="flex items-center gap-x-3">
        ...
        <span
          ...
          :class="
            stat.status === 'up' ? 'text-accent-green' : 'text-accent-orange'
          "
        >
          {{ stat.percentage }}
        </span>
        <div
          ...
          :class="
            stat.status === 'up' ? 'bg-accent-green/20' : 'bg-accent-orange/20'
          "
        >
          <ArrowUpIcon
            v-if="stat.status === 'up'"
